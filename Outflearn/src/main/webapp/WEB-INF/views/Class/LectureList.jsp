...
<script type="text/javascript">
	function PageMove(page, data) {
		location.href = "LectureList?page=" + page + "&txt_search"
...
<body>
					...
					<ul class="nav flex-column text-center">
						...
						<c:forEach items="${mainList }" var="mainDto">
							<li class="nav-item all_category"><a href="void:0"
								id="side_${mainDto.main_num }" class="nav-link active">${mainDto.main_name }&nbsp;<i
									class="fas fa-chevron-right"></i></a>
								<ul id="side_${mainDto.main_num }_menu"
			...
			<div class="col-sm-10">
				<div class="page-header">
					...
					<form name="form1" method="post"
						action="${pageContext.request.contextPath}/LectureList">
						<select name="searchOption">
							<!-- 검색 조건을 검색 처리 후 결과화면에 보여주기 위해 c:out 출력태그 사용, 삼항연산자.-->
							<option value="all"
								<c:out value="${map.searchOption == 'all'?'selected':''}"/>>제목+이름</option>
							<option value="class_author"
								<c:out value="${map.searchOption == 'class_author'?'selected':'' }"/>>이름</option>
							<option value="class_title"
								<c:out value="${map.searchOption == 'class_title'?'selected':''}"/>>제목</option>
						</select> <input type="text" class="form-control" name="txt_search"
							id="txt_search" placeholder="검색하기"> <span
	...
	<div class="form-group form-inline">
		...
		<div class="pages">
			<c:forEach var="i" begin="${pagination.startPageNo}"
				end="${pagination.endPageNo}" step="1">
				<c:choose>
